# ============================================================
# Autor/es: Laura Quintero, Juan Carlos Muñoz
# Fecha de creación: 2024-06
# Descripción: Cálculo de índices de vulnerabilidad por subdimensión
# ============================================================
...
df_aux <- read_csv("04_Cluster_Outputs/Total/datos_map.csv")

# Recortamos la base de datos al grupo 1
df_aux <- subset(df_aux, select = c(nivl_vl, sub_grp))
df_aux_2 <- df_aux %>%
  filter( sub_grp == 3) %>%   # filtra filas
  rename(ind_mpio = nivl_vl)
# Merge 
aux3 <- df_aux_2 %>% left_join(ds, by = "ind_mpio")
aux3 <- aux3 %>% select(-sub_grp)
ds <- aux3
...
idx_1 <- idx_1 %>%
  mutate(
    ...
    ircarural = -1*        ircarural
               ...
               ircaurbano,
               ircarural)  %>%
 reduc_dim(., 1,label, "idx1_servicio")
...
idx_2 <- idx_2 %>%
  ...
  reduc_dim(., 1,label, "idx2_Adultez")
...
idx_3 <- idx_3 %>%
  ...
  reduc_dim(., 1,label, "idx3_climatico")
...
idx_4 <- idx_4 %>%
 select(ind_mpio, nvl_label,
  ...
  reduc_dim(., 1,label, "idx4_Capacidad")
...
idx_5 <- idx_5 %>%
  ...
  reduc_dim(., 1,label, "idx5_viviendas")
...
idx_6 <- idx_6 %>%
  select(ind_mpio, nvl_label,
                ...
                vpc_ter,
                vpc_total)  %>%
  reduc_dim(., 1,label, "idx6_crecimiento")
...
idx_8 <- idx_8 %>%
  mutate(
    ...
    ide_t_r = -1*ide_t_r,
               ...
               ide_t_c,
               ide_t_r)  %>%
  reduc_dim(., 1,label, "idx8_demografica")
...
idx_10 <- idx_10 %>%
  ...
  reduc_dim(., 1,label, "idx10_juventud")
...
idx_11 <- idx_11 %>%
  mutate( 
    ...
    pob_nbi_mujeres = -1*pob_nbi_mujeres
                 ...
                 pob_nbi_hombres,
                 pob_nbi_mujeres)  %>%
  reduc_dim(., 1,label, "idx11_pobreza")
...
idx_13 <- idx_13 %>%
  ...
  reduc_dim(., 1,label, "idx13_saludmental")
...
idx_14 <- idx_14 %>%
  ...
  reduc_dim(., 1,label, "idx14_seguridad")
...
write_xlsx(df_final, path = "01_Data/01_Derived/pca_ds_vulnerable.xlsx")
saveRDS(df_final, file = "01_Data/01_Derived/pca_ds_vulnerable.rds")
