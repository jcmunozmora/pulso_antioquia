...
g_out <- "04_Cluster_Outputs"
tx<- "04_Cluster_Outputs"
library(stargazer)
library(openxlsx)
library(writexl)
library(igraph)
...
#==============================
pca_ds <- read_rds("01_Data/01_Derived/pca_ds_vulnerable.rds") 
    ...
    cat(res, file=glue("04_Cluster_Outputs/Sub_Vulnerable/cluster_desc_sub.tex", sep="\n"))
    cat(res, file=glue("04_Cluster_Outputs/Sub_Vulnerable/cluster_desc_sub.tex", sep="\n"))
  ...
  #==============================
    ...
    ggsave(file=paste0("04_Cluster_Outputs/Sub_Vulnerable/cluster_dist.pdf"), 
  ...
  ggsave(file=paste0("04_Cluster_Outputs/Sub_Vulnerable/cluster_elbow.pdf"),
  ...
  #### Dendograma
  pdf(file = "04_Cluster_Outputs/Sub_Vulnerable/cluster_dendo.pdf", height = h*0.8, width = w)
  fviz_dend(hc5, cex = 0.5, k = 3, rect = TRUE)
  dev.off()
  
  
  #### By Component
  pdf(file = "04_Cluster_Outputs/Sub_Vulnerable/cluster_dendo_vars.pdf", height = h*0.8, width = w) 
  ...
  pdf(file = "04_Cluster_Outputs/Sub_Vulnerable/cluster_dendo_phylogenic.pdf", height = h*0.8, width = w) 
  ...
  #datos_vs_med$sub_grp <- datos_vs_med$sub_grp + 1
    ...
    gathered_datos.j$sub_grp <- factor(gathered_datos.j$sub_grp, 
                                       #levels=c(1,2,3,4),
                                       levels=c(1,2,3),
                                      # labels = c("Consolidada","Emergente","Transición","Vulnerable"))
                                      labels = c("Vulnerable - Alta","Vulnerable - Media","Vulnerable - Bajo"))
    ...
    ggsave(file=paste0("04_Cluster_Outputs/Sub_Vulnerable/cluster_variables_contexto_1.pdf"),
           height = h*0.8, width = w, dpi = 900) 
    ...
    ggsave(file=paste0("04_Cluster_Outputs/Sub_Vulnerable/cluster_variables_contexto_2.pdf"),
           height = h*0.8, width = w, dpi = 900) 
    ...
    ggsave(file=paste0("04_Cluster_Outputs/Sub_Vulnerable/cluster_variables_res_1.pdf"),
           height = h*0.8, width = w, dpi = 900) 
  ...
  ###==============
    ...
    data_map$sub_grp <- factor(data_map$sub_grp,levels=c(1,2,3),labels = c("Vulnerable - Alta","Vulnerable - Media","Vulnerable - Bajo"))
    ...
    ggsave(file=paste0("04_Cluster_Outputs/Sub_Vulnerable/cluster_map.pdf"), 
           g1,height = h, width = w*0.8, dpi = d)
    
    
    datos.j <- as.data.frame(cbind(df, sub_grp))
    datos.j_aux <- tibble::rownames_to_column(as.data.frame(datos.j), var = "municipio")
    write.csv(datos.j_aux, "04_Cluster_Outputs/Sub_Vulnerable/datos_j.csv", row.names = FALSE, na = "", fileEncoding = "UTF-8")
    data_map_aux <- st_drop_geometry(data_map)
    readr::write_csv( data_map_aux, "04_Cluster_Outputs/Sub_Vulnerable/datos_map.csv")
